  ...
  begin
    ...
    try
      ...
      try
        ...
        TLog.NewLog(ltDebug, ClassName, 'Sending job to miners: ' + TBlock.OperationBlockToText
          (P^.OperationsComp.OperationBlock) + ' Cache blocks:' + IntToStr(l.count));
      finally
...
begin
  ...
  try
    ...
    try
      ...
      try
        if (not(TBlock.EqualsOperationBlock(FMinerOperations.OperationBlock, MasterOp.OperationBlock))) then
        begin
          ...
          TLog.NewLog(ltInfo, ClassName, Format('New miner operations:%d Hash:%s %s',
            ...
            TCrypto.ToHexaString(FMinerOperations.OperationBlock.operations_hash)]));
        end
        else
        begin
        ...
        begin
          ...
          begin
            ...
            TLog.NewLog(ltInfo, ClassName, sJobInfo + ' - Found a solution for block ' +
              IntToStr(nbOperations.OperationBlock.Block));
            PowNotOK := false;
          end
          else
            PowNotOK := true;
...
begin
  ...
  try
    ...
    TLog.NewLog(ltInfo, ClassName, Format('Sending job %d to miner - Block:%d Ops:%d Target:%s PayloadStart:%s',
      [nJobs, Operations.OperationBlock.Block, Operations.count, IntToHex(Operations.OperationBlock.compact_target, 8),
      Operations.OperationBlock.block_payload]));
  finally
    params.Free;
  end;
end;
