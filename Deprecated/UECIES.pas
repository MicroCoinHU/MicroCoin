...
int main() {
    ...
    aout = ECDH_compute_key(abuf, alen, EC_KEY_get0_public_key(ecdh2), ecdh, KDF1_SHA1);

    blen = KDF1_SHA1_len;
    bbuf = (unsigned char * )OPENSSL_malloc(blen);
    bout = ECDH_compute_key(bbuf, blen, EC_KEY_get0_public_key(ecdh), ecdh2, KDF1_SHA1);
...
Uses OpenSSLdef, OpenSSL, UCrypto, ULog, UConst;
...
Var PK,PEphemeral : PEC_KEY;
  i,key_length,block_length,body_length : Integer;
  envelope_length: Cardinal;
  mac_length : Cardinal;
...
begin
  ...
  try
    ...
    envelope_length := EC_POINT_point2oct(EC_KEY_get0_group(PEphemeral),
                                          EC_KEY_get0_public_key(PEphemeral),
                                          POINT_CONVERSION_COMPRESSED, nil, 0, nil);
    ...
    try
      ...
      group0 := EC_KEY_get0_group(PEphemeral);
      key0 := EC_KEY_get0_public_key(PEphemeral);
      key_data := secure_key_data(cryptex);
      result_len := EC_POINT_point2oct(group0,
                                       key0,
                                       POINT_CONVERSION_COMPRESSED, key_data, envelope_length, nil);
      If result_len<>envelope_length then begin
